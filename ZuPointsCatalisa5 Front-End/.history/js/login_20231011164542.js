document.addEventListener('DOMContentLoaded', function () {
  const loginForm = document.querySelector('#loginForm');

  loginForm.addEventListener('submit', function (e) {
      e.preventDefault();

      const usuario = document.querySelector('[name="floatingInputUser"]').value;
      const senha = document.querySelector('[name="floatingInputPassword"]').value;

      const loginData = {
          username: usuario,
          password: senha,
      };

      fetch('http://localhost:8080/login/auth', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
          body: JSON.stringify(loginData),
      })
      .then(response => response.json())
      .then(data => {
          console.log('Token de autenticação:', data.token);

          // Armazena o token no localStorage com base no nome de usuário
          localStorage.setItem(`token_${usuario}`, data.token);

          const token = localStorage.getItem(`token_${usuario}`);

          fetch('http://localhost:8080/usuarios', {
              method: 'GET',
              headers: {
                  'Authorization': `Bearer ${token}`
              },
          })
          .then(response => response.json())
          .then(userList => {
              console.log(userList);

              const loggedInUser = userList.find(user => user.username === usuario);
              localStorage.setItem('id', loggedInUser.id)

              if (loggedInUser) {
                  if (loggedInUser.role) {
                      if (loggedInUser.role === 'ROLE_COLABORADOR') {
                          window.location.href = 'colaborador.html';
                      } else if (loggedInUser.role === 'GESTOR') {
                          window.location.href = 'gestor.html';
                      } else {
                          console.error('Função de usuário desconhecida:', loggedInUser.role);
                      }
                  } else {
                      console.error('Usuário não tem uma função definida.');
                  }
              } else {
                  console.error('Usuário não encontrado.');
              }
          })
          .catch(error => {
              console.error('Erro ao obter informações do usuário:', error);
          });
      })
      .catch(error => {
          console.error('Erro na autenticação:', error);
      });
  });
});
Neste código, após o login bem-sucedido, o token é armazenado no localStorage com um nome de chave baseado no nome de usuário do usuário que fez o login. Dessa forma, você pode recuperar o token associado a cada usuário quando necessário. Certifique-se de ajustar seu código e a estrutura de seus dados para usar o nome de usuário como chave no localStorage.





